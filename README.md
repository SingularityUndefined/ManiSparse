...
| Deflation | `DeflationCGSolver` | Runs only on intermediate blocks when `model.deflation.enabled=True` and `ablation!="Theta"`. The first deflated mode is the normal ADMM output. The final block skips deflation and returns the normal ADMM output. It is detached by default and can be made differentiable experimentally. |
...
During training, every 20 batches prints current-batch per-block Theta graph metrics through `tqdm.write`, with the batch header and each block on separate lines. For dense Theta (GLASSO or dense Kalofolias), the line reports off-diagonal `sparsity`, `kNN_recall` against the original main kNN list `model.nearest_nodes[:, 1:]`, and the nonzero outgoing `degree(min/median/max)`. For local Kalofolias, it also reports `kNN_precision`: the fraction of selected local Theta edges that belong to the base kNN graph. Degree is an edge count, not a sum of weights. The same line reports the selected/candidate edge count and each block's `lambda_theta`; scalar values are printed directly, while vector/list values are summarized as `[min, max], median`. Because `Theta` is re-estimated for each batch rather than stored as a persistent model parameter, these values describe the most recent forward pass. Local Kalofolias evaluates support on its local candidate set `N * theta.local_kNN`, while dense Theta evaluates all off-diagonal `N * (N - 1)` entries.

At the end of each epoch, the training log also summarizes the mean `sparsity` and `kNN_recall` across that epoch's every-20-batch Theta samples for each block; local Kalofolias additionally reports mean `kNN_precision`. At training completion, `<loss_curves>/<run>_theta_metrics.png` plots these per-epoch sampled means in three panels (sparsity, recall, and local-only precision), with one curve per unrolled block.
